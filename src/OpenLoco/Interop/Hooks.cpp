...
void OpenLoco::Interop::registerHooks()
{
    ...
    registerHook(
        ...
        [](registers& regs) FORCE_ALIGN_ARG_POINTER -> uint8_t {
            registers backup = regs;
            auto pos = Gfx::drawString((Gfx::drawpixelinfo_t*)(uintptr_t)regs.edi, regs.cx, regs.dx, regs.al, (uint8_t*)(uintptr_t)regs.esi);
            regs = backup;
            regs.cx = pos.x;
            regs.dx = pos.y;
    ...
    registerHook(
        ...
        [](registers& regs) FORCE_ALIGN_ARG_POINTER -> uint8_t {
            registers backup = regs;
            char* buffer = StringManager::formatString((char*)(uintptr_t)regs.edi, regs.eax, (void*)(uintptr_t)regs.ecx);
            regs = backup;
    ...
    registerHook(
        0x004AB655,
        [](registers& regs) -> uint8_t {
            auto v = (Vehicles::VehicleBase*)(uintptr_t)regs.esi;
    ...
    registerHook(
        ...
        [](registers& regs) -> uint8_t {
            ...
            if (widgetIndex == -1)
            {
                regs.edi = (loco_ptr)&window->widgets.get()[0];
            }
            else
            {
                regs.edi = (loco_ptr)&window->widgets.get()[widgetIndex];
            }
